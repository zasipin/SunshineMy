 ...
 */
package app.learn.sunshineex.zasypinnv.sunshinemy.data;
...
public class TestDb extends AndroidTestCase {
    ...
    */
    public void testLocationTable() {
        ...
        Assert.assertTrue("Can't insert record", rowId != -1);

        // Query the database and receive a Cursor back
        Cursor cursor = db.query(WeatherContract.LocationEntry.TABLE_NAME, null, WeatherContract.LocationEntry._ID + " = ?",
                                new String[]{Long.toString(rowId)},null, null, null);

        // Move the cursor to a valid database row
        if(cursor.moveToNext())
        {
            int colIndex = cursor.getColumnIndex(WeatherContract.LocationEntry.COLUMN_LOCATION_SETTING);
            Assert.assertEquals(cursor.getString(colIndex), testLocationSetting);
        }
        ...
        // query if you like)
        cursor.moveToFirst();
        TestUtilities.validateCurrentRecord("Error in data reading", cursor, testValues);
    ...
    public void testWeatherTable() {
        ...
        Assert.assertTrue("Can't insert record", locationRowId != -1);
        ...
        Assert.assertTrue("Can't insert record", weatherRowId != -1);

        // Query the database and receive a Cursor back
        Cursor cursor = db.query(WeatherContract.WeatherEntry.TABLE_NAME,
                                null,
                WeatherContract.WeatherEntry._ID + " = ?",
                                new String[] { Long.toString(weatherRowId) },
                                null, null, null
                                );
        // Move the cursor to a valid database row
        if (cursor.moveToFirst())
        {
            TestUtilities.validateCurrentRecord("Error in data reading", cursor, weatherValues);
        }
        // Validate data in resulting Cursor with the original ContentValues
        // (you can use the validateCurrentRecord function in TestUtilities to validate the
        // query if you like)
